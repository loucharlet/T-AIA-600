...
def clean_summary_words(topic_data, title, author):
    bad_words = {
        "said", "would", "could", "little", "like", "went", "came", "come",
        "looked", "thought", "know", "time", "one", "upon", "people",
        "common", "chapter", "thing", "things", "great", "made", "asked",
        "answered", "first", "last", "much", "many", "every", "quite"
    }

    # enlève aussi les mots du titre et de l'auteur
    title_words = re.sub(r"[^a-z\s]", " ", title.lower()).split()
    author_words = re.sub(r"[^a-z\s]", " ", author.lower()).split()
    bad_words.update(title_words)
    bad_words.update(author_words)
...
def summarize(text, book_id):
    ...
    author = BOOKS.get(book_id, {}).get("author", "Unknown author")

    topic_data = topics(text)
    words = clean_summary_words(topic_data, title, author)

    element1 = words[0] if len(words) > 0 else "adventure"
    element2 = words[1] if len(words) > 1 else "discovery"
    element3 = words[2] if len(words) > 2 else "conflict"

    genre = book_genre(book_id)

    if genre == "children":
        return (
            f'"{title}" by {author} is a children and young adult novel. '
            f'The story is presented as an imaginative adventure, with recurring elements such as {element1}, {element2} and {element3}. '
            f'These elements help create a playful world built around curiosity, encounters and discovery.'
        )

    elif genre == "mystery":
        return (
            f'"{title}" by {author} is a crime and mystery novel. '
            f'The narrative is built around recurring elements such as {element1}, {element2} and {element3}. '
            f'These elements contribute to an atmosphere of investigation, tension and unresolved situations.'
        )

    elif genre == "scifi":
        return (
            f'"{title}" by {author} is a science-fiction and fantasy novel. '
            f'The book develops a speculative atmosphere through recurring elements such as {element1}, {element2} and {element3}. '
            f'It presents a world shaped by danger, discovery and transformation.'
        )

    return (
        f'"{title}" by {author} is built around recurring elements such as '
